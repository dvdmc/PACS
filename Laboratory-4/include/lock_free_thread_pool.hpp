...
class lock_free_thread_pool
{
  ...
  lock_free_queue<std::function<task_type>> _queue; //Pregunta: se puede usar directamente threadsafe_queue<task_type> ? Por qué std::function wrapper?
    ...
    : _done(false), _n_threads(num_threads), _joiner(thread_vector)
  {
      ...
      {
        thread_vector.push_back(std::thread(&lock_free_thread_pool::worker_thread, this)); //Pregunta: se podría poner directamente worker_thread?
      }
  }
