...
int main(int argc, char** argv)
{
  ...
  for (int iteration = 0; iteration < n_iter; iteration++){
    ...
    cl_error(err, "Failed to create memory buffer at device (img)\n");
    cl_mem hist_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_int)*n_bins*channels, NULL, &err);
    cl_error(err, "Failed to create memory buffer at device (hist)\n");
    ...
    size_t global_size[] = {rows*columns + 64-(rows*columns)%64, channels};
