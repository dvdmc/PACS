...
int main(int argc, char** argv)
{
  ...
  FILE *fileHandler = fopen("hist_out_partition.cl", "r");
  ...
  for (int iteration = 0; iteration < n_iter; iteration++){
    ...
    n_bins = 256;
    cl_int hist_gpu[256*3] = {0};

    // Create OpenCL buffer visible to the OpenCl runtime
    cl_mem img_buffer  = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_uint)*channel_size/2, NULL, &err);
    cl_error(err, "Failed to create memory buffer at device\n");
    cl_mem hist_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_int)*n_bins*channels, NULL, &err);
    cl_error(err, "Failed to create memory buffer at device\n");

    cl_mem img_buffer_gpu  = clCreateBuffer(context_gpu, CL_MEM_READ_ONLY, sizeof(cl_uint)*channel_size/2, NULL, &err);
    cl_error(err, "Failed to create memory buffer at device\n");
    cl_mem hist_buffer_gpu = clCreateBuffer(context_gpu, CL_MEM_READ_WRITE, sizeof(cl_int)*n_bins*channels, NULL, &err);
    cl_error(err, "Failed to create memory buffer at device\n");
    
    // Write date into the memory object 
    err = clEnqueueWriteBuffer(command_queue, img_buffer, CL_TRUE, 0, sizeof(cl_uint)*channel_size/2,
                              img_array, 0, NULL, NULL);
    cl_error(err, "Failed to enqueue a write command\n");
    err = clEnqueueWriteBuffer(command_queue, hist_buffer, CL_TRUE, 0, sizeof(cl_int)*n_bins*channels,
                              hist, 0, NULL, NULL);
    cl_error(err, "Failed to enqueue a write command\n");
    err = clEnqueueWriteBuffer(command_queue_gpu, img_buffer_gpu, CL_TRUE, 0, sizeof(cl_uint)*channel_size/2,
                              img_array + sizeof(cl_uint)*channel_size/2, 0, NULL, NULL);
    cl_error(err, "Failed to enqueue a write command\n");
    err = clEnqueueWriteBuffer(command_queue_gpu, hist_buffer_gpu, CL_TRUE, 0, sizeof(cl_int)*n_bins*channels,
    ...
    err = clSetKernelArg(kernel, 4, sizeof(channel_size), &half_channel_size);
    ...
    cl_error(err, "Failed to set argument 3\n");
    err = clSetKernelArg(kernel_gpu, 4, sizeof(channel_size), &half_channel_size);
    cl_error(err, "Failed to set argument 4\n");
    err = clSetKernelArg(kernel_gpu, 5, 256*3*sizeof(unsigned int), NULL);
    cl_error(err, "Failed to set argument 5\n");
    ...
    // Read data form device memory back to host memory
    err = clEnqueueReadBuffer(command_queue_gpu, hist_buffer_gpu, CL_TRUE, 0, sizeof(cl_int)*n_bins*channels, hist_gpu, 0, NULL, NULL);
    cl_error(err, "Failed to enqueue a read command\n");
    ...
    for(int bin = 0; bin < 256*3; bin++){
      hist[bin] += hist_gpu[bin];
    }
  }
