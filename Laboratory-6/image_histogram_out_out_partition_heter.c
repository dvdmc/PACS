...
int main(int argc, char** argv)
{
  ...
  FILE *fileHandler = fopen("hist_out_out_partition.cl", "r");
  ...
  for (int iteration = 0; iteration < n_iter; iteration++){
    ...
    err = clSetKernelArg(kernel, 2, sizeof(group_size), &group_size);
    cl_error(err, "Failed to set argument 2\n");
    err = clSetKernelArg(kernel, 3, sizeof(channel_size), &channel_size);
    cl_error(err, "Failed to set argument 3\n");
    cl_uint min = 0;
    err = clSetKernelArg(kernel, 4, 128*3*sizeof(unsigned int), NULL);
    ...
    cl_error(err, "Failed to set argument 1\n");
    err = clSetKernelArg(kernel_gpu, 2, sizeof(group_size), &group_size_gpu);
    cl_error(err, "Failed to set argument 2\n");
    err = clSetKernelArg(kernel_gpu, 3, sizeof(channel_size), &channel_size);
    cl_error(err, "Failed to set argument 3\n");
    cl_uint min_gpu = 128;
    err = clSetKernelArg(kernel_gpu, 4, 128*3*sizeof(unsigned int), NULL);
    cl_error(err, "Failed to set argument 4\n");
