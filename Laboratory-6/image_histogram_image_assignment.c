...
void cpu_thread(const int n_iter, const char* image_file, int& iterations_cpu, 
double& total_kernel_time){
 int err; 
  ...
  while(iteration.fetch_add(1) < n_iter){
    iterations_cpu++;
...
void gpu_thread(const int n_iter, const char* image_file, int& iterations_gpu,
double& total_kernel_time){
 int err; 
  ...
  // Calculate size of the file
  FILE *fileHandler = fopen("hist_in_partition_vectors.cl", "r");
  fseek(fileHandler, 0, SEEK_END);
  size_t fileSize = ftell(fileHandler);
  rewind(fileHandler);
...
int main(int argc, char** argv)
{
  ...
  int iterations_cpu = 0, iterations_gpu = 0;
  double total_kernel_time, time_cpu = 0, time_gpu = 0;


  std::thread cpu(cpu_thread, n_iter, image_file, ref(iterations_cpu), ref(time_cpu));
  std::thread gpu(gpu_thread, n_iter, image_file, ref(iterations_gpu), ref(time_gpu));

  cpu.join();
  gpu.join();
  total_kernel_time = time_cpu + time_gpu;
  printf("%0.3f\n", total_kernel_time);
  printf("Images CPU: %d->%f,  GPU: %d->%f\n", iterations_cpu, time_cpu, iterations_gpu, time_gpu);
