...
int main(int argc, char** argv)
{
  ...
  FILE *fileHandler = fopen("hist.cl", "r");
  ...
  err = clSetKernelArg(kernel, 2, sizeof(channel_size), &channel_size);
  cl_error(err, "Failed to set argument 2\n");
  err = clSetKernelArg(kernel, 3, sizeof(channels), &channels);
  cl_error(err, "Failed to set argument 3\n");
  // Launch Kernel
  size_t local_size[] = {128, 1};
  size_t global_size[] = {rows*columns, channels};
  ...
  // double bandwidthWrite = (sizeof(cl_uchar)*channel_size*channels + sizeof(cl_int)*n_bins*channels)/((double)time_copy.count());
  // printf("%0.3f\n", bandwidthWrite);
  ...
  // double bandwidthKernel = (sizeof(cl_uchar)*channel_size*channels + sizeof(cl_int)*n_bins*channels)/(nanoSeconds);
  // printf("%0.3f\n", bandwidthKernel);
